...
{{-- ═══════════════════════════════════════════════
     ...
     ═══════════════════════════════════════════════ --}}
<div class="section">
  <div class="section-title">17. GRADING AUDIT</div>
  <div class="section-body">
    @if(!$grading)
      <p class="empty">Belum ada data.</p>
    @else
      <div class="kv-grid" style="margin-bottom:10px;">
        <div class="kv"><span class="kv-label">ID Grading:</span><span class="kv-val">{{ $grading->id_grading ?? '-' }}</span></div>
        <div class="kv"><span class="kv-label">Jenis:</span><span class="kv-val">{{ $grading->jenis ?? '-' }}</span></div>
        <div class="kv"><span class="kv-label">Area:</span><span class="kv-val">{{ $grading->area ?? '-' }}</span></div>
        <div class="kv"><span class="kv-label">Total Nilai:</span><span class="kv-val"><strong>{{ number_format($grading->total_nilai ?? 0, 2, ',', '.') }}</strong></span></div>
        <div class="kv"><span class="kv-label">BBNKB:</span><span class="kv-val">{{ $grading->bbnkb ?? '-' }}</span></div>
        <div class="kv"><span class="kv-label">Fraud:</span><span class="kv-val">{{ $grading->fraud ?? '-' }}</span></div>
      </div>
      @php $details = $grading->details ?? []; @endphp
      @if(count($details))
      <table>
        <thead><tr><th>#</th><th>Item</th><th>Nilai</th><th>Keterangan</th></tr></thead>
        <tbody>
...
{{-- ═══════════════════════════════════════════════
     ...
     ═══════════════════════════════════════════════ --}}
<div class="section">
  <div class="section-title">18. PICA (Problem Identification &amp; Corrective Action)</div>
  <div class="section-body">
    @if($picas->isEmpty())
      <p class="empty">Belum ada PICA.</p>
    @else
      @foreach($picas as $i => $p)
      <div style="border:1px solid #e5e7eb;border-radius:4px;padding:8px;margin-bottom:8px;page-break-inside:avoid;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
          <strong style="font-size:11px;">{{ $p->pica_no ?? ('PICA-'.($i+1)) }} – {{ $p->title ?? '-' }}</strong>
          <span class="badge badge-{{ $p->status ?? 'open' }}">{{ strtoupper($p->status ?? 'open') }}</span>
        </div>
        <div class="kv-grid">
          <div class="kv"><span class="kv-label">Prioritas:</span><span class="kv-val">{{ $p->priority ?? '-' }}</span></div>
          <div class="kv"><span class="kv-label">PIC:</span><span class="kv-val">{{ $p->pic ?? '-' }}</span></div>
          <div class="kv"><span class="kv-label">Target:</span><span class="kv-val">{{ $p->target_date ? \Carbon\Carbon::parse($p->target_date)->format('d/m/Y') : '-' }}</span></div>
          <div class="kv"><span class="kv-label">Realisasi:</span><span class="kv-val">{{ $p->actual_date ? \Carbon\Carbon::parse($p->actual_date)->format('d/m/Y') : '-' }}</span></div>
        </div>
        @if($p->problem)
        <div style="margin-top:5px;"><span class="kv-label">Problem:</span> {{ $p->problem }}</div>
        @endif
        @if($p->current_condition)
        <div style="margin-top:3px;"><span class="kv-label">Kondisi Saat Ini:</span> {{ $p->current_condition }}</div>
        @endif
        @if($p->problem_identification)
        <div style="margin-top:3px;"><span class="kv-label">Tanggapan PICA:</span> {{ $p->problem_identification }}</div>
        @endif
        @if($p->corrective_action)
        <div style="margin-top:3px;"><span class="kv-label">Corrective Action:</span> {{ $p->corrective_action }}</div>
        @endif
        @if($p->preventive_action)
        <div style="margin-top:3px;"><span class="kv-label">Preventive Action:</span> {{ $p->preventive_action }}</div>
        @endif
        @if($p->recheck_note || $p->recheck_deadline)
        <div style="margin-top:5px;background:#fef3c7;padding:4px 8px;border-radius:4px;">
          <strong>Re-Chek:</strong> {{ $p->recheck_note ?? '-' }}
          @if($p->recheck_deadline) · Deadline: {{ \Carbon\Carbon::parse($p->recheck_deadline)->format('d/m/Y') }} @endif
        </div>
        @endif
      </div>
      @endforeach
...
{{-- ═══════════════════════════════════════════════
     ...
     ═══════════════════════════════════════════════ --}}
<div class="section">
  <div class="section-title">19. REKOMENDASI AUDIT</div>
  <div class="section-body">
    @if($rekomendasi->isEmpty())
      <p class="empty">Belum ada rekomendasi.</p>
    @else
      <table>
        <thead><tr><th>#</th><th>Judul</th><th>Isi Rekomendasi</th><th>Kategori</th><th>Prioritas</th><th>Status</th><th>Deadline</th></tr></thead>
        <tbody>
          ...
          <tr>
            ...
            <td>{{ $r->deadline ? \Carbon\Carbon::parse($r->deadline)->format('d/m/Y') : '-' }}</td>
          </tr>
          @endforeach
        </tbody>
      </table>
...
{{-- ═══════════════════════════════════════════════
     ...
     ═══════════════════════════════════════════════ --}}
<div class="section">
  <div class="section-title">20. BU PERFORMANCE</div>
  <div class="section-body">
    @if(!$buPerformance)
      <p class="empty">Belum ada data BU Performance.</p>
    @else
      <div class="kv-grid" style="margin-bottom:8px;">
        <div class="kv"><span class="kv-label">Unit Usaha:</span><span class="kv-val">{{ $buPerformance->unit_usaha ?? '-' }}</span></div>
        <div class="kv"><span class="kv-label">Bulan:</span><span class="kv-val">{{ $buPerformance->bulan ?? '-' }}</span></div>
        <div class="kv"><span class="kv-label">Auditor:</span><span class="kv-val">{{ $buPerformance->auditor ?? '-' }}</span></div>
      </div>
      @php $penilaian = $buPerformance->penilaian ?? []; @endphp
      @if(count($penilaian))
      <table>
        <thead><tr><th>#</th><th>Aspek / KPI</th><th>Nilai</th><th>Keterangan</th></tr></thead>
        <tbody>
...
{{-- ═══════════════════════════════════════════════
     ...
     ═══════════════════════════════════════════════ --}}
<div class="section">
  <div class="section-title">21. LAMPIRAN</div>
  <div class="section-body">
    @if(!$lampiran)
