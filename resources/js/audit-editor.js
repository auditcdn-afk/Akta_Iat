// ── Audit editor (semua tab pemeriksaan) ────────────────────────────────────────
// Modul ini di-load secara dinamis (dynamic import) oleh ./akta-audit.js hanya saat
// user membuka pemeriksaan (klik "Mulai Audit" / "Buka Pemeriksaan", atau via deep
// link ?planId=). Kode di file ini TIDAK diunduh/dijalankan sama sekali selama user
// hanya melihat daftar plan audit — inilah yang membuat halaman Audit jadi jauh
// lebih ringan dibuka dibanding sebelumnya (satu file JS raksasa dimuat penuh untuk
...
export async function openPemeriksaan(plan, deepLinkModul = null) {
    ...
    const modul = plan.isMandiri ? (deepLinkModul || "audit_mandiri") : "audit";
...
function switchTab(tab) {
    ...
    if (tab !== "rsa-hgp" && typeof _flushRsaHgpSaveDebounced === "function") _flushRsaHgpSaveDebounced();
    ...
    if (tab === "rsa-hgp") {
        loadRsaHgpTab().catch((e) => showAlert(e.message, "error"));
    }
    if (tab === "hga") {
...
// ── Init (dijalankan sekali otomatis saat modul ini pertama kali di-import) ──
// DOM sudah pasti siap di sini karena modul ini baru diimpor setelah DOMContentLoaded
// (dipicu oleh klik "Mulai Audit"/"Buka Pemeriksaan" atau deep link di akta-audit.js).
// Evaluasi modul ES hanya terjadi sekali walau di-import berkali-kali, jadi listener di
// bawah ini tidak akan terpasang dobel.
...
function createScanIncrementQueue({ endpoint, simpananKey, delay = 400, maksPercobaan = 10, onItem, onStuck }) {
    // Kunci antrean memuat plan-nya: scan HARUS masuk ke plan tempat ia ditembak,
    // bukan ke plan yang kebetulan sedang dibuka saat request-nya berhasil
    // terkirim. Auditor bisa berpindah plan (openPemeriksaan) selagi ada scan
    // tertinggal karena jaringan putus.
    const pending  = new Map();   // `${planId}|${noPart}` → { planId, noPart, qty, idx, extra, entries, percobaan }
    const terkirim = new Map();   // `${batch}|${planId}|${noPart}` → item yang sama
    let timer = null;
    let chain = Promise.resolve();
    let gagalBeruntun = 0;
    let batchKe = 0;

    const kunci = (planId, noPart) => `${planId}|${noPart}`;
    const semua = () => [...terkirim.values(), ...pending.values()];
    ...
    // dan dipulihkan saat tab dibuka lagi. Aman diulang karena tiap entri bawa id
    // sendiri: server melewati id yang sudah tercatat (terapkanEntriScan), jadi
    // kiriman ulang tidak pernah menghitung dobel.
    const kunciSimpanan = (planId) => `${simpananKey}:${planId}`;

    function simpanAntrean() {
        if (!simpananKey) return;
        try {
            // Yang sedang dikirim ikut disimpan sampai server mengonfirmasi:
            // halaman bisa mati persis di tengah request.
            const perPlan = new Map();
            for (const p of semua()) {
                if (!p.planId || !p.entries.length) continue;
                if (!perPlan.has(p.planId)) perPlan.set(p.planId, []);
                perPlan.get(p.planId).push([p.noPart, { qty: p.qty, extra: p.extra, entries: p.entries, percobaan: p.percobaan }]);
            }
            // Plan yang antreannya sudah habis dibersihkan dari penyimpanan.
            for (let i = localStorage.length - 1; i >= 0; i--) {
                const k = localStorage.key(i);
                if (k?.startsWith(`${simpananKey}:`) && !perPlan.has(Number(k.slice(simpananKey.length + 1)))) {
                    localStorage.removeItem(k);
                }
            }
            perPlan.forEach((isi, planId) => localStorage.setItem(kunciSimpanan(planId), JSON.stringify(isi)));
    ...
    // sebelumnya (plan mana pun) ke antrean, lalu kirim. Entri yang id-nya sudah
    // ada di memori dilewati supaya membuka tab berkali-kali tidak menggandakan
    // antrean.
    function pulihkanAntrean() {
        if (!simpananKey) return 0;
        const idDiMemori = new Set();
        semua().forEach(p => p.entries.forEach(e => e.id && idDiMemori.add(e.id)));
        ...
        try {
            for (let i = 0; i < localStorage.length; i++) {
                const k = localStorage.key(i);
                if (k?.startsWith(`${simpananKey}:`)) kunciTersimpan.push(k);
            }
        } catch (_) { return 0; }

        for (const k of kunciTersimpan) {
            const planId = Number(k.slice(simpananKey.length + 1));
            if (!planId) continue;
            let isi = [];
            try { isi = JSON.parse(localStorage.getItem(k) || '[]'); } catch (_) { continue; }
            ...
            for (const baris of isi) {
                ...
                const entriBaru = p.entries.filter(e => e && e.id && !idDiMemori.has(e.id));
                if (!entriBaru.length) continue;
                entriBaru.forEach(e => idDiMemori.add(e.id));
                kembalikan({
                    planId,
                    noPart: String(noPart),
                    // qty dihitung ulang dari entri yang benar-benar dipulihkan.
                    qty: entriBaru.reduce((n, e) => n + (Number(e.qty) || 0), 0),
                    idx: -1,
                    extra: p.extra && typeof p.extra === 'object' ? p.extra : {},
                    entries: entriBaru,
                    percobaan: 0,
                });
                dipulihkan += entriBaru.length;
            }
        }
        if (dipulihkan) {
    ...
    function kembalikan(p) {
        const k = kunci(p.planId, p.noPart);
        const cur = pending.get(k);
        if (!cur) { pending.set(k, p); return; }
        cur.qty += p.qty;
        cur.entries = p.entries.concat(cur.entries);
        cur.extra = { ...p.extra, ...cur.extra };
        cur.percobaan = Math.max(cur.percobaan, p.percobaan || 0);
    }
    ...
    function flush({ keepalive = false } = {}) {
        ...
        const tanda = ++batchKe;
        pending.clear();
        // Kunci unik per batch: dua flush beruntun untuk No. Part yang sama tidak
        // boleh saling menimpa slotnya, karena yang lebih dulu selesai akan
        // menghapus entri milik batch yang lebih baru.
        ...
        chain = chain.then(async () => {
            ...
            for (const p of batch) {
                ...
                try {
                    const body = { planAuditId: p.planId, noPart: p.noPart, qty: p.qty, ...p.extra };
                    ...
                    terkirim.delete(kTerkirim);
                    simpanAntrean();
                    onItem?.(res, p.idx, p.noPart, p.planId);
                } catch (err) {
                    terkirim.delete(kTerkirim);
                    p.percobaan = (p.percobaan || 0) + 1;
                    // 404/422 pun TIDAK pernah dibuang: "No. Part tidak ditemukan"
                    // dan "Data belum ada untuk plan ini" bisa sementara (import
                    // baru berjalan, part baru ditambahkan perangkat lain). Yang
                    // dilakukan cuma berhenti mencoba otomatis setelah sekian kali
                    // sambil memberi tahu auditor — datanya tetap tersimpan dan
                    // dicoba lagi saat tab dibuka atau ada scan berikutnya.
    ...
    return {
        // scanId dibuat pemanggil supaya entri logScan optimistis di layar memakai
        // id YANG SAMA dengan yang dikirim ke server — kalau tidak, simpan-penuh
        // bisa menimpa log ber-id milik server dan kiriman ulang jadi dobel.
        push(noPart, qty, idx, extra = {}, scanId = null) {
            if (!activePlanId || !noPart) return;
            const k = kunci(activePlanId, noPart);
            let p = pending.get(k);
            if (!p) {
                p = { planId: activePlanId, noPart, qty: 0, idx, extra: {}, entries: [], percobaan: 0 };
                pending.set(k, p);
            }
            p.idx = idx;
            ...
            if (qty !== 0) {
                p.qty += qty;
                p.entries.push({ id: scanId || SCAN_ID(), at: new Date().toISOString(), qty });
            }
        ...
        // ada scan yang belum terkirim untuk part ini (di plan yang sedang dibuka),
        // angka server sudah basi.
...
// Cari baris berdasarkan No. Part PERSIS (bukan pencocokan longgar seperti
// hgpFindIdx yang dipakai saat scan) — ini untuk menulis balik balasan server
// ke baris yang benar.
function hgpIndeksNoPart(noPart) {
    const items = _hgpData?.items || [];
    const cari = (noPart || '').trim().toLowerCase();
    if (!cari) return null;
    const k = items.findIndex(it => (it.noPart || '').trim().toLowerCase() === cari);
    return k >= 0 ? k : null;
}
...
const _hgpScanQueue = createScanIncrementQueue({
    ...
    onItem: (res, idx, noPart, planId) => {
        // Balasan bisa datang setelah daftar itemnya berubah (import ulang menata
        // ulang & menambah item), jadi barisnya dicari ulang lewat No. Part —
        // indeks yang ditangkap saat scan bisa sudah menunjuk item lain. Balasan
        // untuk plan lain diabaikan sama sekali.
        if (planId && planId !== activePlanId) return;
        const i = (hgpIndeksNoPart(noPart) ?? idx);
        ...
        if (!res?.item || !_hgpData?.items?.[i]) return;
...
function _doScanHgpIncrement(noPart, qty, idx, extra = {}, scanId = null) {
    _hgpScanQueue.push(noPart, qty, idx, extra, scanId);
}
...
function hgpScanAccumulate(code) {
    ...
    // simpan-penuh bisa menimpa log ber-id milik server sehingga kiriman ulang
    // tidak lagi terdeteksi duplikat (fisiknya jadi dobel).
    const scanId = SCAN_ID();
    it.logScan.push({ id: scanId, at: new Date().toISOString(), qty: 1 }); // history per scan
    it.tgl = document.getElementById('hgpFormTgl')?.value || it.tgl;
    hgpCalcItem(it);

    hgpUpdateSingleRow(idx);
    _doScanHgpIncrement(it.noPart, 1, idx, {}, scanId);
...
function hgpFormSaveEntry() {
    ...
    it.tgl = tgl;
    if (!Array.isArray(it.logScan)) it.logScan = [];
    const scanId = SCAN_ID();   // lihat catatan id scan di hgpScanAccumulate()
    it.logScan.push({ id: scanId, at: new Date().toISOString(), qty });
    hgpCalcItem(it);
    hgpUpdateSingleRow(idx);
    _doScanHgpIncrement(it.noPart, qty, idx, { keterangan, tgl }, scanId);
...
async function loadHgpTab() {
    ...
    // ingatan browser. Pertahankan yang di layar — antreannya masih mencoba
    // kirim ulang. Tapi kalau layarnya memang belum punya data sama sekali
...
/* ============================================================
   RSA HGP & AHM Oils
   ============================================================ */
let _rsaHgpData = null; // { items: [ { sparepart, saldoAkhir, fisik, akhir, selisih, keterangan, tgl, logScan:[] } ] }

function rsaHgpEmptyData() { return { items: [] }; }
...
// Saldo baseline = saldo akhir sistem (fallback ke field lama saldoAwal)
function rsaHgpSaldo(item) {
    return rsaHgpN(item?.saldoAkhir ?? item?.saldoAwal);
}

function rsaHgpCalcItem(item) {
    const fisik   = rsaHgpN(item.fisik);
    const wo      = rsaHgpN(item.wo);           // WO menambah fisik
    const total   = fisik + wo;
    const saldo   = rsaHgpSaldo(item);
    item.akhir    = saldo - total;           // Akhir = saldo - (fisik + wo)
    item.selisih  = total - saldo;           // Selisih = (fisik + wo) - saldo
}

function rsaHgpUpdateStats() {
    const items   = _rsaHgpData?.items || [];
    const total   = items.length;
    const selisih = items.filter(it => it.selisih !== 0).length;
    const scan    = items.reduce((s, it) => s + (it.logScan?.length || 0), 0);
    const el = id => document.getElementById(id);
    if (el('rsaHgpStatTotal'))   el('rsaHgpStatTotal').textContent   = total;
    if (el('rsaHgpStatSelisih')) el('rsaHgpStatSelisih').textContent = selisih;
    if (el('rsaHgpStatScan'))    el('rsaHgpStatScan').textContent    = scan;
...
async function rsaHgpEnrichWithHet(items) {
    if (!items || !items.length) return;
    const kodes = [...new Set(items.map(it => it.noPart).filter(Boolean))];
    if (!kodes.length) return;
    try {
        ...
        items.forEach(it => {
            ...
            if (hit) {
                if (!it.sparepart || it.sparepart === it.noPart) it.sparepart = hit.nama || it.sparepart;
                it.hargaHet = rsaHgpN(hit.hargaHet);
            }
        });
    } catch (_) {}
}

async function rsaHgpHandleFile(file) {
    const msg = document.getElementById('rsaHgpImportMsg');
    if (msg) { msg.classList.remove('hidden'); msg.textContent = 'Mengupload...'; }
    try {
        const fd = new FormData();
        fd.append('file', file);
        fd.append('planAuditId', activePlanId || '');
        const res = await fetchJson('/api/audit-detail/rsa-hgp/parse-excel', {
            method: 'POST',
            headers: authHeaders(),
            body: fd,
        });
        ...
        // RSA = Random Sampling Audit: server hanya mengembalikan sample (30 item normal,
        // 50 item untuk unit usaha ber-role WHS) dari total item yang ditemukan di file,
        // bukan seluruh data — beda dengan tool HGP & AHM Oils biasa yang memuat semua item.
        const sampleNote = res.sampled
            ? ` — disampling otomatis ${res.data.length} dari ${res.totalFound} item ditemukan (ukuran sample: ${res.sampleSize}).`
            : '';
        ...
        // Replace: import = master data baru. Pertahankan fisik & logScan untuk noPart yang cocok.
        const prevByPart = {};
        (_rsaHgpData.items || []).forEach(it => {
            if (it.noPart) prevByPart[it.noPart.toLowerCase()] = it;
        });
        _rsaHgpData.items = res.data.map(it => {
            const prev = it.noPart ? prevByPart[it.noPart.toLowerCase()] : null;
            if (prev) {
                it.fisik   = rsaHgpN(prev.fisik);
                it.logScan = Array.isArray(prev.logScan) ? prev.logScan : [];
                rsaHgpCalcItem(it);
            }
            return it;
        });
        _rsaHgpData.totalDitemukan = res.totalFound;
...
/* ---- Form pemeriksaan (scan / dropdown No. Part) ---- */
let _rsaHgpSelIdx = -1; // index item yang sedang dipilih di form
...
// sendiri (murah, di memory) dan datalist cuma diisi HASIL SARINGAN yang dibatasi
// jumlahnya, bukan seluruh data import.
const RSA_HGP_DATALIST_MAX = 40;
function rsaHgpPopulateDatalist(filterTerm) {
    const dl = document.getElementById('rsaHgpPartList');
    if (!dl) return;
    const items = _rsaHgpData?.items || [];
    const esc = s => (s || '').replace(/"/g, '&quot;');
    const term = (filterTerm || '').trim().toLowerCase();
    let list = term
        ? items.filter(it => (it.noPart || '').toLowerCase().includes(term) || (it.sparepart || '').toLowerCase().includes(term))
        : items;
    list = list.slice(0, RSA_HGP_DATALIST_MAX);
    dl.innerHTML = list.map(it => {
        const noPart = esc(it.noPart || it.sparepart || '');
        const nama   = esc(it.sparepart || '');
        // Tampilkan No. Part sebagai nilai utama; nama part jadi keterangan
...
function rsaHgpFormSelectPart(code) {
    ...
    const ketEl = document.getElementById('rsaHgpFormKet');
    const tglEl = document.getElementById('rsaHgpFormTgl');
    if (qtyEl) qtyEl.value = 0;  // qty ini untuk scan baru, bukan total
    if (ketEl) ketEl.value = it.keterangan || '';
    if (tglEl && it.tgl) tglEl.value = it.tgl;
    rsaHgpFormRecalc();
...
// jaringan gudang yang belum tentu kencang.
// Cari baris berdasarkan No. Part PERSIS (bukan pencocokan longgar seperti
// rsaHgpFindIdx yang dipakai saat scan) — ini untuk menulis balik balasan server
// ke baris yang benar.
function rsaHgpIndeksNoPart(noPart) {
    const items = _rsaHgpData?.items || [];
    const cari = (noPart || '').trim().toLowerCase();
    if (!cari) return null;
    const k = items.findIndex(it => (it.noPart || '').trim().toLowerCase() === cari);
...
const _rsaHgpScanQueue = createScanIncrementQueue({
    ...
    onItem: (res, idx, noPart, planId) => {
        ...
        // untuk plan lain diabaikan sama sekali.
        if (planId && planId !== activePlanId) return;
        const i = (rsaHgpIndeksNoPart(noPart) ?? idx);
        // Selaraskan dengan hasil otoritatif server (jaga-jaga ada device lain yang
        // ikut scan No. Part yang sama di waktu yang berdekatan). Kalau masih ada
        // scan yang belum terkirim untuk part ini, nilai server sudah ketinggalan —
        // biarkan angka lokal yang optimistis sampai antrean berikutnya terkirim.
        if (!res?.item || !_rsaHgpData?.items?.[i]) return;
...
function _doScanRsaHgpIncrement(noPart, qty, idx, extra = {}, scanId = null) {
    _rsaHgpScanQueue.push(noPart, qty, idx, extra, scanId);
}

// Scan barcode: akumulasi fisik +1 setiap scan kode yang sama, tiap scan tercatat di log.
// Setelah scan, form dikosongkan total agar siap scan berikutnya.
let _rsaHgpScanGuard = false;
function rsaHgpScanAccumulate(code) {
    const msg  = document.getElementById('rsaHgpFormMsg');
    const showMsg = (text, ok) => {
        if (!msg) return;
        msg.classList.remove('hidden');
        msg.textContent = text;
    ...
    _doScanRsaHgpIncrement(it.noPart, 1, idx, {}, scanId);

    // Pesan hasil scan, lalu kosongkan form untuk scan berikutnya
    showMsg(`✓ ${it.noPart || it.sparepart} — ${it.sparepart} | Fisik: ${rsaHgpN(it.fisik)} | Akhir: ${it.akhir} | Selisih: ${it.selisih >= 0 ? '+' : ''}${it.selisih} (Terscan: ${it.logScan.length})`, true);
    rsaHgpFormClearInputs();
}

// Kosongkan input form (tanpa menyentuh pesan hasil scan)
function rsaHgpFormClearInputs() {
    _rsaHgpSelIdx = -1;
    const partInput = document.getElementById('rsaHgpFormPart');
    const qtyEl = document.getElementById('rsaHgpFormQty');
    const ketEl = document.getElementById('rsaHgpFormKet');
    const akhirEl = document.getElementById('rsaHgpFormAkhir');
    const selEl = document.getElementById('rsaHgpFormSelisih');
    const info  = document.getElementById('rsaHgpFormPartInfo');
    const log   = document.getElementById('rsaHgpFormLog');
    document.getElementById('rsaHgpFormFields')?.classList.add('hidden');
    if (partInput) {
        _rsaHgpScanGuard = true;
        partInput.value = '';
        setTimeout(() => { _rsaHgpScanGuard = false; partInput.focus(); }, 0);
    }
    if (qtyEl)  qtyEl.value = 0;
    if (ketEl)  ketEl.value = '';
    if (akhirEl) akhirEl.value = 0;
...
function rsaHgpFormSaveEntry() {
    ...
    rsaHgpCalcItem(it);
    rsaHgpUpdateSingleRow(idx);
    _doScanRsaHgpIncrement(it.noPart, qty, idx, { keterangan, tgl }, scanId);
    showMsg(`✓ Tersimpan: ${it.noPart || it.sparepart} — Fisik ${rsaHgpN(it.fisik)}, Akhir ${it.akhir}, Selisih ${it.selisih >= 0 ? '+' : ''}${it.selisih}`, true);
    rsaHgpFormClearInputs();
}

function rsaHgpFormReset() {
    _rsaHgpSelIdx = -1;
    const ids = ['rsaHgpFormPart', 'rsaHgpFormKet'];
    ids.forEach(id => { const e = document.getElementById(id); if (e) e.value = ''; });
    const qty = document.getElementById('rsaHgpFormQty'); if (qty) qty.value = 0;
    const info = document.getElementById('rsaHgpFormPartInfo'); if (info) info.textContent = '';
    const msg = document.getElementById('rsaHgpFormMsg'); if (msg) msg.classList.add('hidden');
    rsaHgpFormRecalc();
}
...
// Tampilkan info sampling secara persisten (bukan cuma sesaat setelah import),
// supaya auditor yang buka ulang tab ini tetap tahu berapa total item ditemukan
// vs berapa yang disampling untuk discan.
function rsaHgpUpdateSampleInfo() {
    const el = document.getElementById('rsaHgpSampleInfo');
    if (!el) return;
    const total = _rsaHgpData?.totalDitemukan;
    const size  = _rsaHgpData?.sampleSize;
...
function initRsaHgpForm() {
    ...
    dropzone?.addEventListener('drop', e => {
        ...
        dropzone.classList.remove('border-blue-400');
        const f = e.dataTransfer.files[0];
        if (f) rsaHgpHandleFile(f);
    });

    document.getElementById('rsaHgpSaveBtn')?.addEventListener('click', () => {
        saveRsaHgp().catch(err => showAlert(err.message || 'Gagal menyimpan.', 'error'));
    });
    ...
    // Pilih dari dropdown → load (tanpa akumulasi). Scan + Enter → akumulasi fisik +1.
    partInput?.addEventListener('change', () => {
        if (_rsaHgpScanGuard) return;
        if (partInput.value) rsaHgpFormSelectPart(partInput.value);
    });
    partInput?.addEventListener('keydown', e => {
        if (e.key === 'Enter') { e.preventDefault(); rsaHgpScanAccumulate(partInput.value); }
    });
    // Saring ulang datalist tiap ketik supaya isinya selalu kecil (lihat komentar di
    ...
    document.getElementById('rsaHgpFormQtyInc')?.addEventListener('click', () => {
    ...
    document.getElementById('rsaHgpFormResetBtn')?.addEventListener('click', () => rsaHgpFormReset());
    ...
    // Tambah Part Manual
    const addPartBtn    = document.getElementById('rsaHgpAddPartBtn');
    const addPartForm   = document.getElementById('rsaHgpAddPartForm');
    const addPartNo     = document.getElementById('rsaHgpAddPartNo');
    const addPartNama   = document.getElementById('rsaHgpAddPartNama');
    const addPartSave   = document.getElementById('rsaHgpAddPartSave');
    const addPartCancel = document.getElementById('rsaHgpAddPartCancel');
    const addPartMsg    = document.getElementById('rsaHgpAddPartMsg');
    ...
    document.getElementById('rsaHgpClearBtn')?.addEventListener('click', () => {
        if (!confirm('Hapus semua data RSA HGP & AHM Oils? Data lama akan dikosongkan, lalu import ulang file Excel.')) return;
        _rsaHgpData = rsaHgpEmptyData();
        rsaHgpRenderItems();
        rsaHgpPopulateDatalist();
        rsaHgpFormReset();
        const msg = document.getElementById('rsaHgpImportMsg');
...
// Cari baris berdasarkan No. Part PERSIS (bukan pencocokan longgar seperti
// hgaFindIdx yang dipakai saat scan) — ini untuk menulis balik balasan server
// ke baris yang benar.
function hgaIndeksNoPart(noPart) {
    const items = _hgaData?.items || [];
    const cari = (noPart || '').trim().toLowerCase();
    if (!cari) return null;
    const k = items.findIndex(it => (it.noPart || '').trim().toLowerCase() === cari);
...
const _hgaScanQueue = createScanIncrementQueue({
    ...
    onItem: (res, idx, noPart, planId) => {
        ...
        // untuk plan lain diabaikan sama sekali.
        if (planId && planId !== activePlanId) return;
        const i = (hgaIndeksNoPart(noPart) ?? idx);
        if (!res?.item || !_hgaData?.items?.[i]) return;
...
function _doScanHgaIncrement(noPart, qty, idx, extra = {}, scanId = null) {
    _hgaScanQueue.push(noPart, qty, idx, extra, scanId);
}
...
function hgaScanAccumulate(code) {
    ...
    const scanId = SCAN_ID();
    it.logScan.push({ id: scanId, at: new Date().toISOString(), qty: 1 });
    it.tgl = document.getElementById('hgaFormTgl')?.value || it.tgl;
    hgaCalcItem(it);
    hgaUpdateSingleRow(idx);
    _doScanHgaIncrement(it.noPart, 1, idx, {}, scanId);
...
function hgaFormSaveEntry() {
    ...
    hgaCalcItem(it);
    hgaUpdateSingleRow(idx);
    _doScanHgaIncrement(it.noPart, qty, idx, { keterangan, tgl }, scanId);
...
window.rekomendasiIsiStep      = rekomendasiIsiStep;
