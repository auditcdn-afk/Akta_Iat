        <div id="tabPanel-rsa-hgp" class="audit-tab-panel hidden space-y-5">
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button id="rsaHgpSaveBtn" type="button"
            ...
            <div id="rsaHgpDropzone"
                ...
                <p class="text-sm text-slate-300">
                    Drag &amp; drop file <span class="font-semibold text-blue-400">.xls / .xlsx / .csv</span> data RSA HGP &amp; AHM Oils ke sini, atau
                </p>
                <label class="inline-flex cursor-pointer items-center gap-2 rounded-xl bg-yellow-500 px-4 py-2 text-sm font-semibold text-slate-900 shadow hover:bg-yellow-400 transition">
                    📁 Pilih File Excel
                    <input type="file" id="rsaHgpFileInput" accept=".xls,.xlsx,.csv" class="hidden">
                </label>
                <p id="rsaHgpImportMsg" class="hidden text-sm font-medium text-green-400"></p>
            </div>

            {{-- Info: RSA = Random Sampling Audit --}}
            <div class="rounded-xl border border-blue-700/40 bg-blue-900/10 px-4 py-3 text-xs text-blue-300">
                ℹ️ <strong>RSA (Random Sampling Audit)</strong> — sistem otomatis hanya mengambil sample <strong>30 item</strong>
                (atau <strong>50 item</strong> untuk unit usaha ber-role WHS) dari total data yang diimport, bukan seluruh data.
                Item yang perlu discan hanyalah hasil sample tersebut.
            </div>
            <p id="rsaHgpSampleInfo" class="hidden rounded-xl border border-amber-700/40 bg-amber-900/10 px-4 py-3 text-xs font-medium text-amber-300"></p>

            {{-- Stat --}}
            <div class="grid grid-cols-3 gap-4">
                <div class="rounded-2xl border border-slate-700 bg-slate-800/60 p-4 text-center">
                    <p id="rsaHgpStatTotal" class="text-2xl font-bold text-slate-100">0</p>
                    <p class="mt-1 text-xs uppercase tracking-wide text-slate-400">Total Sparepart</p>
                </div>
                <div class="rounded-2xl border border-slate-700 bg-slate-800/60 p-4 text-center">
                    <p id="rsaHgpStatSelisih" class="text-2xl font-bold text-red-400">0</p>
                    <p class="mt-1 text-xs uppercase tracking-wide text-slate-400">Selisih</p>
                </div>
                <div class="rounded-2xl border border-slate-700 bg-slate-800/60 p-4 text-center">
                    <p id="rsaHgpStatScan" class="text-2xl font-bold text-green-400">0</p>
            ...
            <div class="rounded-2xl border border-blue-700/40 bg-blue-900/10 p-4">
                <div class="mb-3 flex items-center justify-between">
                    ...
                    <button id="rsaHgpAddPartBtn" type="button"
                ...
                <div id="rsaHgpAddPartForm" class="hidden mb-4 rounded-xl border border-emerald-700/40 bg-emerald-900/10 p-4 space-y-3">
                    <p class="text-xs font-semibold text-emerald-300">Tambah No. Part di luar data import</p>
                    <div class="flex gap-2">
                        <input type="text" id="rsaHgpAddPartNo" placeholder="No. Part (wajib)"
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <input type="date" id="rsaHgpFormTgl"
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <input type="text" id="rsaHgpFormPart" list="rsaHgpPartList" autocomplete="off"
                            placeholder="Scan barcode atau pilih No. Part..."
                            class="rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-sm text-slate-100 focus:border-blue-400 focus:outline-none">
                        <datalist id="rsaHgpPartList"></datalist>
                        <p id="rsaHgpFormPartInfo" class="mt-0.5 text-xs text-slate-400"></p>
                    </div>

                    {{-- Fields berikut hanya muncul setelah No. Part dipilih --}}
                    <div id="rsaHgpFormFields" class="hidden space-y-3">

                        {{-- Qty --}}
                        <div class="flex flex-col gap-1">
                            <label class="text-xs font-semibold text-slate-300">Qty (Fisik Scan) <span class="text-red-400">*</span></label>
                            <div class="flex items-center gap-2">
                                <button id="rsaHgpFormQtyDec" type="button" class="rounded-lg bg-slate-700 px-3 py-1.5 text-slate-200 hover:bg-slate-600">−</button>
                                <input type="number" id="rsaHgpFormQty" value="0"
                                ...
                                <button id="rsaHgpFormQtyInc" type="button" class="rounded-lg bg-slate-700 px-3 py-1.5 text-slate-200 hover:bg-slate-600">+</button>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div class="flex flex-col gap-1">
                                ...
                                <input type="text" id="rsaHgpFormAkhir" readonly value="0"
                            ...
                            <div class="flex flex-col gap-1">
                                ...
                                <input type="text" id="rsaHgpFormSelisih" readonly value="0"
                                    class="rounded-lg border border-slate-700 bg-slate-800/60 px-3 py-2 text-sm text-slate-300">
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <input type="text" id="rsaHgpFormKet" placeholder="Keterangan..."
                                class="rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-sm text-slate-100 focus:border-blue-400 focus:outline-none">
                        </div>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <div id="rsaHgpFormLog" class="rounded-lg border border-slate-700 bg-slate-800/60 px-3 py-2 text-xs text-slate-400">
                                Fisik Terscan : 0 | Saldo Akhir : -
                            </div>
                        </div>

                        <div class="flex items-center gap-2 pt-1">
                            <button id="rsaHgpFormSaveBtn" type="button"
                            ...
                            <button id="rsaHgpFormResetBtn" type="button"
                    ...
                    <p id="rsaHgpFormMsg" class="text-xs font-medium"></p>
                    {{-- Peringatan kalau ada hasil scan yang belum sampai ke server;
                         diisi oleh rsaHgpSetSyncWarn() di audit-editor.js. --}}
                    <p id="rsaHgpSyncWarn" class="hidden mt-1 rounded-lg border border-amber-600/50 bg-amber-900/20 px-3 py-2 text-xs font-semibold text-amber-300"></p>
                </div>
            </div>
            ...
            <div id="rsaHgpTableSection" class="overflow-hidden rounded-2xl border border-slate-700 bg-slate-900">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full min-w-[1500px] text-xs">
                        ...
                        <tbody id="rsaHgpTableBody" class="divide-y divide-slate-800/60"></tbody>
                    </table>
                </div>
            </div>
